package com.practice.login.filter;
...
/**
 * 스프링 시큐리티의 폼 기반의 UsernamePasswordAuthenticationFilter를 참고하여 만든 커스텀 필터 거의 구조가 같고, Type이 Json인
...
public class CustomJsonUsernamePasswordAuthenticationFilter extends
    ...
    private static final AntPathRequestMatcher DEFAULT_LOGIN_PATH_REQUEST_MATCHER =
        new AntPathRequestMatcher(DEFAULT_LOGIN_REQUEST_URL,
            HTTP_METHOD); // "/login" + POST로 온 요청에 매칭된다.
    ...
    public Authentication attemptAuthentication(HttpServletRequest request,
        ...
        UsernamePasswordAuthenticationToken authRequest = UsernamePasswordAuthenticationToken.unauthenticated(
            email, password);//principal 과 credentials 전달

        // authentication 객체(Username~Token)에 detail을 set함. (ip, sessionId를 set하는 것으로 보임)
